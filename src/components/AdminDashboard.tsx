...
import { ChildAdminSection } from './ChildAdminSection';
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const loadChildren = async () => {
      setChildrenState((current) => ({ ...current, error: null, isLoading: true }));
      try {
  ...
  useEffect(() => {
    ...
    const loadStories = async () => {
      setStoriesState((current) => ({ ...current, error: null, isLoading: true }));
      try {
  ...
  return (
    <ScrollView contentContainerStyle={styles.page}>
      <View style={styles.headerCopy}>
        <Text accessibilityRole="header" style={styles.heading}>Administration</Text>
        <Text style={styles.introduction}>
          Maintain children and review story reference data. This desktop
          layout is a presentation choice, not a security or authorization
          boundary.
        </Text>
      </View>
      ...
      <View style={styles.catalogGrid}>
        <ChildAdminSection
          ...
          onRefresh={() => setChildrenAttempt((current) => current + 1)}
        />
        <View style={styles.section}>
          <Text accessibilityRole="header" style={styles.sectionHeading}>Stories</Text>
          <Text style={styles.sectionDescription}>
            Stories remain read-only in this checkpoint.
          </Text>
          {storiesState.isLoading ? (
            ...
            <View style={styles.state}>
              <Text style={styles.recordTitle}>No stories are configured.</Text>
            </View>
          ) : (
              ...
              {sortedStories.map((story) => (
                ...
                >
                  ...
                  <Text style={styles.metadata}>
                    {story.theme} · {story.readingMinutes} min · Story ID {story.id}
                  </Text>
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  page: { alignSelf: 'center', gap: theme.spacing.lg, maxWidth: 1200, padding: theme.spacing.xl, width: '100%' },
  headerCopy: { maxWidth: 760 },
  heading: { color: theme.colors.textPrimary, fontSize: 30, fontWeight: '700' },
  introduction: { color: theme.colors.textSecondary, fontSize: 16, lineHeight: 24, marginTop: theme.spacing.sm },
  catalogGrid: { alignItems: 'flex-start', flexDirection: 'row', gap: theme.spacing.lg },
  section: { backgroundColor: theme.colors.surface, borderColor: theme.colors.border, borderRadius: theme.radius.lg, borderWidth: 1, flex: 1, minWidth: 0, padding: theme.spacing.lg },
  ...
  row: { backgroundColor: theme.colors.surfaceRaised, borderColor: theme.colors.border, borderRadius: theme.radius.sm, borderWidth: 1, padding: theme.spacing.md },
  recordTitle: { color: theme.colors.textPrimary, fontSize: 17, fontWeight: '700' },
  recordSummary: { color: theme.colors.textSecondary, lineHeight: 20, marginTop: theme.spacing.xs },
  metadata: { color: theme.colors.textSecondary, fontSize: 13, fontWeight: '600', marginTop: theme.spacing.sm },
  ...
  errorText: { color: theme.colors.error, fontSize: 17, fontWeight: '700' },
  retryButton: { alignItems: 'center', backgroundColor: theme.colors.primary, borderRadius: theme.radius.md, justifyContent: 'center', minHeight: 44, paddingHorizontal: theme.spacing.lg },
  primaryButtonText: { color: theme.colors.background, fontWeight: '700' },
  pressedButton: { backgroundColor: theme.colors.primaryPressed },
  desktopRequired: { alignItems: 'center', alignSelf: 'center', justifyContent: 'center', maxWidth: 600, minHeight: '100%', padding: theme.spacing.xl },
});
